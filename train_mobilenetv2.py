...
def main():
    ...
    # 默认使用最后一块 GPU，最大可能保证其空闲
    device = torch.device("cuda:{}".format(GPU_count - 1) if torch.cuda.is_available() else "cpu")
    print("Using cuda:{0}/{1} device training.".format(GPU_count - 1, [item for item in range(GPU_count)]))
    ...
    VOC_root = "/home/shaolun/PYTHON/object-detection/faster-rcnn.pytorch/data/"
    # check voc root
    ...
    train_data_set = VOC2007DataSet(VOC_root, data_transform["train"], "train.txt")
